...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 hover:border-blue-200">
      ...
      <div className="relative h-64 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        <div className="absolute top-4 left-4 flex flex-row gap-2">
          {product.original_price && discountPercentage > 0 && (
            <Badge className="bg-gradient-to-r from-red-500 to-red-600 text-white font-bold text-sm px-3 py-1 shadow-lg">
              -{discountPercentage}%
            </Badge>
          )}
          {product.featured && (
            <Badge className="bg-gradient-to-r from-yellow-400 to-yellow-500 text-yellow-900 font-bold text-sm px-3 py-1 shadow-lg">
              ⭐ Featured
            </Badge>
          )}
        </div>
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
          <Button
            size="sm"
            variant="secondary"
            className="w-11 h-11 p-0 rounded-full shadow-xl bg-white/90 backdrop-blur-sm hover:bg-white border-0 hover:scale-110 transition-all duration-200"
          >
        ...
        {!product.in_stock && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center backdrop-blur-sm">
            <Badge
              variant="destructive"
              className="text-white font-bold px-4 py-2 text-base shadow-lg"
            >
              Out of Stock
            </Badge>
          </div>
      ...
      <div className="p-6">
        ...
        <Badge
          variant="outline"
          className="mb-3 text-xs font-semibold bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100 transition-colors"
        >
          {product.category}
        </Badge>
